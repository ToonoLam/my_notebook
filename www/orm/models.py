...
from config import COOKIE_KEY, COOKIE_NAME
from orm import Model
from orm.db_fields import *

StringField = functools.partial(StringField, ddl='varchar(50)')  # 偏函数，默认传入ddl='varchar(50)'


def next_id():
    return '%015d%s000' % (int(time.time() * 1000), uuid.uuid4().hex)  # 生成随机的主键值
...
class User(Model):
    ...
    email = StringField()  # 由于前面使用函数式编程的偏函数,使得StringField默认传入ddl='varchar(50)'
    ...
    async def save(self):
        self.id = next_id()
        sha1_pw = '%s:%s' % (self.id, self.password)  # 使用随机id值加密原始密码
